...
void BSTreeLevelOrder (BSTree t)
{
	...
	Queue bst = newQueue();
	QueueJoin(bst, t);
	while (!QueueIsEmpty(bst)) {
		Item leave = QueueLeave(bst);
		showBSTreeNode (leave);
		if (leave->left != NULL) QueueJoin(bst, leave->left);
		if (leave->right != NULL) QueueJoin(bst, leave->right);
	}
	dropQueue(bst);
}
