...
void IntListInsertInOrder (IntList L, int v)
{
	...
	if (L->first == NULL){
		...
		// go throgh the cy linked list
		for(struct IntListNode *curr=L->first,*temp;curr!=NULL;curr = curr->next){
			// deal with change happened in the front
			if(curr==L->first){
				if (curr->data > v){
					L->size++;
					temp= newIntListNode(v);
					L->first = temp;
					temp->next = curr;
					break;
				}
			}
			// deal with exchange happened in the middle
			if(curr->next!=NULL && curr->next->data > v){ // ! be aware condition
				temp = curr->next;
				curr->next = newIntListNode(v);
				curr->next->next = temp;
				L->size++;
				break;
			}else if(curr==L->last){ //deal with change in the last
				IntListInsert(L,v);
				break;
			}
